...
| `.github/workflows/*` | `.github/workflows/ci.yml`, `.github/workflows/publish.yml`, `scripts/release-guards.test.sh`, `Cargo.lock` | CI/CD -> Rust CI/publish workflows, exact pull-request-head checkout assertions, immutable sibling and upstream Go revisions, a checked-in dependency lock used by fail-closed sibling source-identity verification, focused native Windows library/TTY validation, example parity, and trusted default-branch badge publication |
